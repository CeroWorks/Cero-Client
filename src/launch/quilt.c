...
int parse_quilt_spec(const char* spec, char* mc, size_t mcsz,
    ...
    loader[0] = '\0';
    if (strncmp(spec, "quilt:", 6) != 0) return 0;
    const char* p = spec + 6;
...
int fetch_quilt_profile(const char* client_dir,
    ...
    if (loader_version[0] == '\0') {
        ...
        snprintf(list_url, sizeof(list_url),
                "%s/versions/loader/%s", QUILT_META, mc_version);

        char list_path[MAX_PATH_SIZE];
        snprintf(list_path, sizeof(list_path),
                "%s/cache/quilt_loaders_%s.json", client_dir, mc_version);
        ensure_parent_dirs(list_path);

        if (download_file(list_url, list_path) == 0) {
            log_msg("error", "Cannot fetch Quilt loader list for %s\n", mc_version);
            return 0;
        }
        ...
        if (!p) { free(txt); log_msg("error", "No Quilt loader found in list\n"); return 0; }
        p += strlen("\"version\"");
        while (*p == ' ' || *p == ':' || *p == '\t') p++;
        ...
        log_msg("info", "Using latest Quilt loader: %s\n", loader_version);
    }

    snprintf(out_id, out_id_sz, "quilt-loader-%s-%s",
    ...
    if (download_file(profile_url, out_json_path) == 0) {
        log_msg("error", "Cannot download Quilt profile from %s\n", profile_url);
        return 0;
    }
